...
__interrupt static void r_adc_interrupt(void)
{
    ...
    //stop adc
    R_ADC_Get_Result(&sensor_comps.adc_result[ADS][sensor_comps.adc_pos]);
    ADS++;
    if(ADS<MD_ADC_CHANNEL_MAX_NUM)
    {
	   R_ADC_Start(); 
    }
    else
    {
        ADS=0;
        sensor_comps.adc_pos++;
//	if(sensor_comps.adc_pos>=MD_ADC_MAX_POS)
//	{
//		sensor_comps.sw|=MD_ADC_EOC_FLAG;
//	}
    }
    /* End user code. Do not edit comment generated here */
}
